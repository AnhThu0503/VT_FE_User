...
const ProductDiscountAll = () => {
  const [products, setProducts] = useState([]);
  const [sortedProducts, setSortedProducts] = useState([]);

  useEffect(() => {
    getProductDiscountAll();
  }, []);
  const getProductDiscountAll = async () => {
    try {
      const response = await axios.get("/api/products-discount-all");
    ...
    >
      <div className="container products-container px-5 pt-4">
        <div className="d-flex" style={{ justifyContent: "space-between" }}>
          <p className="title-primary">Tất cả sản phẩm khuyến mãi</p>
...
export default ProductDiscountAll;
